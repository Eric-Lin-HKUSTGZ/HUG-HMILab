...
class GraspDataset(Dataset):
    ...
    def _sample_point_from_mask(
        ...
        if valid.sum() == 0:
            ...
            if len(ys) == 0:
                # Mask fully empty (e.g. target outside the cropped view):
                # fall back to the image center with mean valid depth.
                H, W = mask_np.shape
                v_pix, u_pix = H // 2, W // 2
            else:
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if stored_uv is not None:
            ...
            # Local-window median depth (robust to single-pixel holes); the
            # old single-pixel read fell back to the whole-image mean when the
            # query pixel was a hole, anchoring the PCL crop on background.
            win = depth_np[
                max(0, vi - 7) : vi + 8, max(0, ui - 7) : ui + 8
            ]
            win = win[win > 0]
